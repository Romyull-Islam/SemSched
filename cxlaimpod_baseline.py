...
# Mimics: CXLAimPod (Yang et al., 2025)
# Fix: Corrected Throughput Calculation (Tokens/s vs Layers/s)
...
def simulate_cxlaimpod(layers, is_prefill):
    prefetcher = CXLPrefetcher() 
...
pf_time = simulate_cxlaimpod(layers, True)
dec_time = simulate_cxlaimpod(layers, False)

# --- FIX IS HERE ---
# dec_time is the TOTAL latency for one token (all layers included).
# We do NOT divide by len(layers) again.
tps = 1.0 / dec_time 

# Prefill TPS (512 tokens processed in pf_time)
