...
# The default "2 * params" is too low, making the simulation memory-bound.
# A larger value makes compute time a significant factor, so core allocation matters.
...
# Hot layers to pin in Host DRAM first (same frequency, but lm_head has huge bytes)
