...
# ==============================
layers = build_layers(sequence_length=512)  # Uses DEFAULT_MODEL_CFG
...
for n in HOT_LAYERS_BY_NAME:  # Use HOT_LAYERS_BY_NAME for placement experiments
    idx = name_to_idx.get(n)
    if idx is not None:
        sz = layers[idx]["bytes"]  # Only parameters for placement
        if sz <= host_free:
...
if first_decoder_idx != -1:
    for i in range(first_decoder_idx, first_decoder_idx + num_blocks):
        sz = layers[i]["bytes"]  # Only parameters for placement
        if sz <= host_free:
...
total_model_bytes = sum(L["bytes"] for L in layers)  # Only parameters for cold load
total_kv_cache_bytes = sum(L.get("kv_cache_bytes", 0) for L in layers)
model_dtype_bits = int(BYTES_PER_PARAM * 8)

host_bytes = sum(layers[i]["bytes"] + layers[i].get("kv_cache_bytes", 0) for i in range(len(layers)) if placement[i] == PL_HOST_DRAM)
ssd_bytes  = sum(layers[i]["bytes"] + layers[i].get("kv_cache_bytes", 0) for i in range(len(layers)) if placement[i] == PL_HOST_SSD)
...
# ==============================
rows = []
# --- CORRECTED LOGIC: Cold load is for parameters only ---
cold_load_s = ssd_cold_time_s(total_model_bytes)

# --- NEW: Calculate incremental KV cache size per token ---
sequence_length = 512  # Must match build_layers call
...
for exec_idx in range(len(layers)):
    ...
    if model_fits_in_dram:
        ...
        if place == PL_HOST_DRAM:
            ...
            mem_dram = 0.0; mem_ssd = ssd_sequential_time_s(sz)  # Manual offloading
...
print("Model fits in DRAM." if model_fits_in_dram else "Model does NOT fit in DRAM → sequential SSD streaming.")
